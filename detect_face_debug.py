...
def detect_face(img, minsize, pnet, rnet, onet, threshold, factor):
    ...
    for j in range(len(scales)):
        ...
        print("  scale:", scale, "\t", im_data.shape, "tensor shape", img_y.shape)
        out = pnet(img_y)
        print(" out = pnet(tensor)", [o.shape for o in out])
        ...
        print(" boxes after non minimum supression", total_boxes.shape)
    ...
    numbox = total_boxes.shape[0]
    print("Second stage (RNET) on %d boxes"%numbox)
    if numbox>0:
        # second stage
        ...
        print(" tensor", tempimg1.shape)
        out = rnet(tempimg1)
        print(" out = rnet(tensor)", [o.shape for o in out])
    ...
    numbox = total_boxes.shape[0]
    print("Third stage (ONET) on %d boxes"%numbox)
    if numbox>0:
        # third stage
        ...
        print(" tensor", tempimg1.shape)
        out = onet(tempimg1)
        print(" out = onet(tensor)", [o.shape for o in out])
        out0 = np.transpose(out[0])
        out1 = np.transpose(out[1])
        ...
        if total_boxes.shape[0]>0:
            ...
            total_boxes = total_boxes[pick,:]
            points = points[:,pick]
        print(" total_boxes after NMS", total_boxes.shape)
        print(" points after NMS", points.shape)
